...
int main(int argc, char * argv[]) {    
    ...
    TFIDF::TFIDF_ tf_idf{true, dataset + "/training-data.csv", dataset + "/testing-data.txt", dataset + "/testing-correct-data.txt", 
                         "tests/output/results/parallel-1024-" + std::to_string(ds_num) + "-results.txt", 
                         "tests/output/processed-data-results/parallel-1024-" + std::to_string(ds_num) + "-processed.csv",
