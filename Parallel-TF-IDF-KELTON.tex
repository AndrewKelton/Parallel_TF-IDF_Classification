...
% To find the most similar category to an untrained (uncategorized) document, I iterate through all \code{Category} objects in the \code{std::vector} of categories. In this report, I used 5 category types:
...
\begin{itemize}
    \item \code{process_trained_data} %\lstinline[language=C]|process_trained_data|
    \item \code{process_un_trained_data}% \lstinline[language=C]|process_un_trained_data|
...
\section{Testing}
To accurately compare the results against the sequential C++ implementation, I used the same original dataset and split it into 80% training data and 20% untrained data. This ensured consistent input across all versions of the program. I measured execution time for each section by starting a counter at the beginning of the function call and stopping it immediately after the function returned. Execution time was reported in milliseconds. Below I will present the results of performance, accuracy, and precision analysis.
