...
int main(int argc, char * argv[]) {    
    ...
    TFIDF::TFIDF_ tf_idf{true, dataset + "/training-data.csv", dataset + "/un-trained-data.txt", dataset + "/un-trained-correct-data.txt", 
                            "tests/test-output/results/parallel-4-" + std::to_string(ds_num) + "-results.txt", 
                            "tests/test-output/processed-data-results/parallel-4-" + std::to_string(ds_num) + "-processed.csv",
